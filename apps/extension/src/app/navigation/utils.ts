...
export async function recreateOnboardingTab(): Promise<void> {
  ...
  const url = `onboarding.html#/${TopLevelRoutes.Onboarding}`
 ...
 * we only update the URL of the active tab if it's already in a specific route of the Poki interface.
 *
 * If the current tab is not in that route, we open a new tab instead.
 */
export async function focusOrCreateInterfaceTab({
  url,
...
export async function focusOrCreateTokensExploreTab({ currencyId }: { currencyId: string }): Promise<void> {
  ...
  return focusOrCreateInterfaceTab({
    url,
    // We want to reuse the active tab only if it's already in any other TDP.
    // eslint-disable-next-line security/detect-non-literal-regexp
    reuseActiveTabIfItMatches: new RegExp(`^${escapeRegExp(pokiWalletUrls.webInterfaceTokensUrl)}`),
  })
}
...
export async function focusOrCreateNftItemTab({
  ...
  return focusOrCreateInterfaceTab({
    url: `${pokiWalletUrls.webInterfaceNftItemUrl}/${address}/${tokenId}`,
    // We want to reuse the active tab only if it's already in any other NFT item page.
    // eslint-disable-next-line security/detect-non-literal-regexp
    reuseActiveTabIfItMatches: new RegExp(`^${escapeRegExp(pokiWalletUrls.webInterfaceNftItemUrl)}`),
  })
}
