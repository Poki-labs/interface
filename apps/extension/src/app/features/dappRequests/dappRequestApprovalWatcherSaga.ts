  ...
  try {
    if (type === confirmRequest.type) {
      ...
      switch (confirmedRequest.dappRequest.type) {
        ...
        case DappRequestType.OpenSidebar: {
          const validatedRequest: OpenSidebarRequest = OpenSidebarRequestSchema.parse(confirmedRequest.dappRequest)
          yield* call(handleOpenSidebarRequest, validatedRequest, confirmedRequest.senderTabInfo)
          break
        }
      }
