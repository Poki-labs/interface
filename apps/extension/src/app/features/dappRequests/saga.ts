...
import { hexadecimalStringToInt, toSupportedChainId } from 'poki/src/features/chains/utils'
...
export function* handleRequest(requestParams: DappRequestNoDappInfo) {
  if (requestParams.dappRequest.type === DappRequestType.OpenSidebar) {
...
export function* handleOpenSidebarRequest(request: OpenSidebarRequest, senderTabInfo: SenderTabInfo) {
  if (request.tab) {
    yield* call(navigate, {
  ...
  const response: OpenSidebarResponse = {
