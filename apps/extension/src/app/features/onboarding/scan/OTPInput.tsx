...
import { Flex, Input, Square, Text, inputStyles } from 'ui/src'
...
export function OTPInput(): JSX.Element {
  ...
  const submitOTP = useCallback(async (): Promise<void> => {
    ...
    const response = await fetch(`${pokiWalletUrls.scantasticApiUrl}/otp`, {
      method: 'POST',
      headers: {
    ...
    if (!data.encryptedSeed) {
      ...
      throw new Error(`fetch(${pokiWalletUrls.scantasticApiUrl}/otp failed to include an encrypted seed`)
    }
