...
import { DOT_GRID, POKI_LOGO } from 'ui/src/assets'
...
const LOGO_SIZE = 52
const LOGO_SCALE_LOADING = 1.2
const LOGO_SCALE_DEFAULT = 1
...
export function ScanToOnboard(): JSX.Element {
  ...
  const checkOTPState = useCallback(async (): Promise<void> => {
    ...
    try {
      // poll OTP state
      const response = await fetch(`${pokiWalletUrls.scantasticApiUrl}/otp-state/${sessionUUID}`, {
        method: 'POST',
        headers: {
  ...
  const qrScale = useSharedValue(LOGO_SCALE_DEFAULT)
  useEffect(() => {
    if (!isLoadingUUID) {
      qrScale.value = LOGO_SCALE_DEFAULT
      return undefined
    }
    ...
    qrScale.value = withRepeat(
      withSequence(withSpring(LOGO_SCALE_LOADING, springConfig), withSpring(LOGO_SCALE_DEFAULT, springConfig)),
      0,
      true,
    )
          ...
          >
            {errorDerivingQR ? (
               ...
               */}
                ...
                >
                  <Image height={iconSizes.icon40} source={POKI_LOGO} width={iconSizes.icon40} />
                </AnimatedFlex>
                {isLoadingUUID ? (
