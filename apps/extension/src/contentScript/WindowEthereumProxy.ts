...
const messages = {
  errors: {
    disconnected: (): string => 'Poki Wallet: Disconnected from chain. Attempting to connect.',
    invalidRequestArgs: (): string => `Poki Wallet: Expected a single, non-array, object argument.`,
    invalidRequestGeneric: (): string => `Poki Wallet: Please check the input passed to the request method`,
  },
}
...
export class WindowEthereumProxy extends EventEmitter {
  /**
   * Boolean indicating that the provider is Poki Wallet.
   */
  isPokiWallet = true

  /**
   * Boolean to spoof MetaMask
   * TODO(EXT-393): Remove this once more dapps support EIP-6963 or have explicit support for Poki Wallet.
   */
  isMetaMask: boolean
