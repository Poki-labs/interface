...
import { PokiMethodHandler } from 'src/contentScript/methodHandlers/PokiMethodHandler'
...
const pokiMethodHandler = new PokiMethodHandler(
  getChainId,
  getProvider,
  getConnectedAddresses,
  setChainIdAndMaybeEmit,
...
addWindowMessageListener<WindowEthereumRequest>(isValidWindowEthereumRequest, async (request, source) => {
  ...
  if (isPokiMethod(request.method)) {
    try {
      await pokiMethodHandler.handleRequest(request, source)
    } catch (e) {
