...
export function isPokiMethod(method: string): boolean {
  return Object.keys(PokiMethods).includes(method)
}
...
export function postDeprecatedMethodError(source: MessageEventSource | null, requestId: string, method: string): void {
  source?.postMessage({
    ...
    error: serializeError(
      providerErrors.unsupportedMethod(`Poki Wallet does not support ${method} as it is deprecated`),
    ),
  })
}
...
export function postUnknownMethodError(source: MessageEventSource | null, requestId: string, method: string): void {
  source?.postMessage({
    requestId,
    error: serializeError(providerErrors.unsupportedMethod(`Poki Wallet does not support ${method}`)),
  })
}
...
export function postParsingError(source: MessageEventSource | null, requestId: string, method: string): void {
  source?.postMessage({
    requestId,
    error: serializeError(
      providerErrors.unsupportedMethod(`Poki Wallet could not parse the ${method} request properly`),
    ),
  })
}
