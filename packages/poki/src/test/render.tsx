...
import { UrlContext } from 'poki/src/contexts/UrlContext'
import { SharedPersistQueryClientProvider } from 'poki/src/data/apiClients/SharedPersistQueryClientProvider'
import { UnitagUpdaterContextProvider } from 'poki/src/features/unitags/context'
import 'poki/src/i18n'
import { PokiState, pokiReducer } from 'poki/src/state/pokiReducer'
import { AutoMockedApolloProvider } from 'poki/src/test/mocks'
...
type ExtendedRenderOptions = RenderOptions & {
  cache?: InMemoryCache
  resolvers?: Resolvers
  preloadedState?: PreloadedState<PokiState>
  store?: EnhancedStore<PokiState>
}
    ...
    return (
      <AutoMockedApolloProvider cache={cache} resolvers={resolvers}>
        <ReduxProvider store={store}>
          <SharedPokiProvider>{children}</SharedPokiProvider>
        </ReduxProvider>
...
function SharedPokiProvider({ children }: Pick<TamaguiProviderProps, 'children'>): JSX.Element {
  return (
