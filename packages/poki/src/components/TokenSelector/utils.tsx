...
import { SafetyLevel as GqlSafetyLevel } from 'poki/src/data/graphql/poki-data-api/__generated__/types-and-hooks'
import { GetSwappableTokensResponse, SafetyLevel } from 'poki/src/data/tradingApi/__generated__'
...
import { areCurrencyIdsEqual } from 'poki/src/utils/currencyId'
...
function tokenOptionComparator(tokenOption: TokenOption, otherTokenOption: TokenOption): boolean {
  return areCurrencyIdsEqual(tokenOption.currencyInfo.ledger_id, otherTokenOption.currencyInfo.ledger_id)
}
...
export function mergeSearchResultsWithBridgingTokens(
  ...
  const extractedSearchResults = searchResults.map((section) => {
    ...
    section.data.forEach((token) => {
      ...
      //   areCurrencyIdsEqual(token.currencyId, bridgingToken.currencyInfo.currencyId),
      // )
