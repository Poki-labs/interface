...
import { useAllCommonBaseCurrencies } from 'poki/src/components/TokenSelector/hooks/useAllCommonBaseCurrencies'
import { useCommonTokensOptionsWithFallback } from 'poki/src/components/TokenSelector/hooks/useCommonTokensOptionsWithFallback'
...
import { useFavoriteTokensOptions } from 'poki/src/components/TokenSelector/hooks/useFavoriteTokensOptions'
...
import { Chain, SafetyLevel } from 'poki/src/data/graphql/poki-data-api/__generated__/types-and-hooks'
...
const preloadedState: PreloadedState<PokiState> = {
  favorites: {
...
describe(useFilterCallbacks, () => {
  ...
  describe('search filter', () => {
    ...
    it('parses chain from search filter', async () => {
      ...
      await act(() => {
        result.current.onChangeText('BaSE poki')
      })

      expect(result.current.chainFilter).toEqual(null)
      expect(result.current.searchFilter).toEqual('BaSE poki')
      expect(result.current.parsedChainFilter).toEqual(UniverseChainId.Base)
      expect(result.current.parsedSearchFilter).toEqual('poki')
    })
    ...
    it('does not parse unsupported chains', async () => {
      ...
      await act(() => {
        result.current.onChangeText('UNSUPPORTED poki')
      })

      expect(result.current.chainFilter).toEqual(null)
      expect(result.current.searchFilter).toEqual('UNSUPPORTED poki')
    ...
    it('only parses after the first space', async () => {
      ...
      await act(() => {
        result.current.onChangeText('base poki corn')
      })

      expect(result.current.chainFilter).toEqual(null)
      expect(result.current.searchFilter).toEqual('base poki corn')
      expect(result.current.parsedChainFilter).toEqual(UniverseChainId.Base)
      expect(result.current.parsedSearchFilter).toEqual('poki corn')
    })
  })
...
jest.mock('poki/src/data/rest/tokenRankings', () => ({
