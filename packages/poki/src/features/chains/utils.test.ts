...
import { SUPPORTED_CHAIN_IDS, SUPPORTED_TESTNET_CHAIN_IDS, UniverseChainId } from 'poki/src/features/chains/types'
import {
  ...
  toPokiWebAppLink,
  toSupportedChainId,
} from 'poki/src/features/chains/utils'
...
describe(fromPokiWebAppLink, () => {
  it('handles supported chain', () => {
    expect(fromPokiWebAppLink(Chain.Ethereum.toLowerCase())).toEqual(UniverseChainId.Mainnet)
    expect(fromPokiWebAppLink(Chain.Arbitrum.toLowerCase())).toEqual(UniverseChainId.ArbitrumOne)
    expect(fromPokiWebAppLink(Chain.Optimism.toLowerCase())).toEqual(UniverseChainId.Optimism)
    expect(fromPokiWebAppLink(Chain.Polygon.toLowerCase())).toEqual(UniverseChainId.Polygon)
    // TODO: add Base test once Chain includes Base (GQL reliant)
  })

  it('handle unsupported chain', () => {
    expect(() => fromPokiWebAppLink('unkwnown')).toThrow('Network "unkwnown" can not be mapped')
  })
})

describe(toPokiWebAppLink, () => {
  it('handles supported chain', () => {
    expect(toPokiWebAppLink(UniverseChainId.Mainnet)).toEqual(Chain.Ethereum.toLowerCase())
    expect(toPokiWebAppLink(UniverseChainId.ArbitrumOne)).toEqual(Chain.Arbitrum.toLowerCase())
    expect(toPokiWebAppLink(UniverseChainId.Optimism)).toEqual(Chain.Optimism.toLowerCase())
    expect(toPokiWebAppLink(UniverseChainId.Polygon)).toEqual(Chain.Polygon.toLowerCase())
    // TODO: add Base test once Chain includes Base (GQL reliant)
  })
