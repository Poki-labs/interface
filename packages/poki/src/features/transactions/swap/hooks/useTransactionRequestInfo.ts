...
import { WithV4Flag } from 'poki/src/data/apiClients/tradingApi/TradingApiClient'
...
import { GasFeeResult, areEqualGasStrategies } from 'poki/src/features/gas/types'
...
import { getBaseTradeAnalyticsPropertiesFromSwapInfo } from 'poki/src/features/transactions/swap/analytics'
...
import { SwapGasFeeEstimation } from 'poki/src/features/transactions/swap/types/swapTxAndGasInfo'
import { ApprovalAction, TokenApprovalInfo } from 'poki/src/features/transactions/swap/types/trade'
...
// TODO(PokiX): add fallback gas limits per chain? l2s have higher costs
...
export function useTransactionRequestInfo({
  ...
  const swapRequestArgs: WithV4Flag<CreateSwapRequest> | undefined = useMemo(() => {
    // TODO: MOB(2438) https://linear.app/poki/issue/MOB-2438/poki-x-clean-old-trading-api-code
    if (!swapQuote) {
      return undefined
    }
  ...
  const isPokiXWrap = trade && isPokiX(trade) && trade.needsWrap
  const isWrapApplicable = derivedSwapInfo.wrapType !== WrapType.NotApplicable || isPokiXWrap
