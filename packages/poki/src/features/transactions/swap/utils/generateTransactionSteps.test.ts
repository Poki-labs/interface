...
import { IndependentToken, ProtocolItems, Routing, TradeType } from 'poki/src/data/tradingApi/__generated__'
import {
  IncreasePositionTxAndGasInfo,
  LiquidityTransactionType,
  LiquidityTxAndGasInfo,
} from 'poki/src/features/transactions/liquidity/types'
import { TransactionStepType } from 'poki/src/features/transactions/swap/types/steps'
import { PokiXSwapTxAndGasInfo, SwapTxAndGasInfo } from 'poki/src/features/transactions/swap/types/swapTxAndGasInfo'
import { ClassicTrade } from 'poki/src/features/transactions/swap/types/trade'
import { generateTransactionSteps } from 'poki/src/features/transactions/swap/utils/generateTransactionSteps'
import { mockPermit } from 'poki/src/test/fixtures/permit'
import {
  createMockCurrencyAmount,
  createMockPokiXTrade,
  createMockTradeWithStatus,
} from 'poki/src/test/fixtures/transactions/swap'
...
describe('Swap', () => {
  ...
  const mockPokiXTrade = createMockPokiXTrade(USDC, WBTC)
  ...
  describe(Routing.DUTCH_V2, () => {
    it('should return steps for pokix trade', () => {
      const swapTxContext: PokiXSwapTxAndGasInfo = {
        ...baseSwapTxContext,
        trade: mockPokiXTrade,
      ...
      expect(generateTransactionSteps(swapTxContext)).toEqual([
        {
          ...
          amount: mockPokiXTrade.inputAmount,
        },
        {
          ...swapTxContext.permit,
          type: TransactionStepType.PokiXSignature,
          quote: swapTxContext.trade.quote.quote,
          deadline: mockPokiXTrade.quote.quote.orderInfo.deadline,
        },
      ])
    })

    it('should return steps for pokix trade with revocation and approval required', () => {
      const swapTxContext: PokiXSwapTxAndGasInfo = {
        ...baseSwapTxContext,
        trade: mockPokiXTrade,
        routing: Routing.DUTCH_V2,
      ...
      expect(generateTransactionSteps(swapTxContext)).toEqual([
        ...
        {
          amount: mockPokiXTrade.inputAmount.quotient.toString(),
          spender: '0x000000000022d473030f116ddee9f6b43ac78ba3',
          txRequest: swapTxContext.approveTxRequest,
          token: USDC,
          type: TransactionStepType.TokenApprovalTransaction,
    ...
    it('should return steps for pokix trade with approval required', () => {
      const swapTxContext: PokiXSwapTxAndGasInfo = {
        ...baseSwapTxContext,
        trade: mockPokiXTrade,
        routing: Routing.DUTCH_V2,
