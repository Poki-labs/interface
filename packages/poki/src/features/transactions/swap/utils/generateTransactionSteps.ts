...
import { parseErrorMessageTitle } from 'poki/src/features/transactions/liquidity/utils'
import {
  ClassicSwapFlow,
  ClassicSwapSteps,
...
import { SwapTxAndGasInfo, isValidSwapTxContext } from 'poki/src/features/transactions/swap/types/swapTxAndGasInfo'
import { isBridge, isClassic, isPokiX } from 'poki/src/features/transactions/swap/utils/routing'
import {
  ValidatedPermit,
  ValidatedTransactionRequest,
  validateTransactionRequest,
} from 'poki/src/features/transactions/swap/utils/trade'
import { Currency, CurrencyAmount, Token } from 'poki/src/sdk-core'
import { parseERC20ApproveCalldata } from 'poki/src/utils/approvals'
...
function orderPokiXSteps(flow: PokiXSwapFlow): PokiXSwapSteps[] {
  const steps: PokiXSwapSteps[] = []
...
function createSignOrderPokiXStep(
  permitData: ValidatedPermit,
  quote: DutchQuoteV2 | DutchQuoteV3 | PriorityQuote,
): PokiXSignatureStep {
  return { type: TransactionStepType.PokiXSignature, deadline: quote.orderInfo.deadline, quote, ...permitData }
}
...
export function generateTransactionSteps(
  ...
  if (isValidLP) {
        ...
        if (txContext.unsigned) {
          return orderMigrateLiquiditySteps({
            permit: createPermit2SignatureStep(
              ...
              new Token(1, ADDRESS_ZERO, 1, 'Poki V3 Positions NFT-V1', 'Poki V3 Positions NFT-V1'),
            ),
    ...
    } else if (isPokiX(txContext)) {
      return orderPokiXSteps({
        revocation,
        approval,
        wrap: createWrapTransactionStep(txContext.wrapTxRequest, trade.inputAmount),
        signOrder: createSignOrderPokiXStep(txContext.permit, txContext.trade.quote.quote),
      })
