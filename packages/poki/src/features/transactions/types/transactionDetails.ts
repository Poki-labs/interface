...
import { TransactionListQuery } from 'poki/src/data/graphql/poki-data-api/__generated__/types-and-hooks'
import { Routing, TransactionFailureReason } from 'poki/src/data/tradingApi/__generated__/index'
...
import { DappInfo } from 'poki/src/types/walletConnect'
...
interface BaseTransactionDetails extends TransactionId {
  ...
  addedTime: number
  // Note: hash is mandatory for classic transactions and undefined for unfilled PokiX orders
...
export interface PokiXOrderDetails extends BaseTransactionDetails {
  ...
  // PokiX orders will also have a transaction `hash` if they become filled.
...
export type TransactionDetails = PokiXOrderDetails | OnChainTransactionDetails
...
export type FinalizedTransactionDetails = TransactionDetails &
  (
    ...
    | {
        ...
        hash?: string // Hash may be undefined for non-successful transactions, as the pokix backend does not provide hashes for cancelled, failed, or expired orders.
      }
  )
...
export interface WrapTransactionInfo extends BaseTransactionInfo {
  ...
  // Currently, this will only be set for wraps that are part of a PokiX native-input swap.
  swapTxId?: string
}
