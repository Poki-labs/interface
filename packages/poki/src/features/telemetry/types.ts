...
import { Currency, TradeType } from 'poki/src/sdk-core'
...
} from 'analytics-events/src/index'
import { TokenOptionSection } from 'poki/src/components/TokenSelector/types'
import { NftStandard } from 'poki/src/data/graphql/poki-data-api/__generated__/types-and-hooks'
...
import { UnitagClaimContext } from 'poki/src/features/unitags/types'
import { RenderPassReport } from 'poki/src/types/RenderPassReport'
import { CurrencyField } from 'poki/src/types/currency'
import { LimitsExpiry } from 'poki/src/types/limits'
import { ImportType } from 'poki/src/types/onboarding'
import { ExtensionOnboardingFlow } from 'poki/src/types/screens/extension'
import { SwapTab } from 'poki/src/types/screens/interface'
import { ShareableEntity } from 'poki/src/types/sharing'
import { EthMethod, UwULinkMethod, WCEventType, WCRequestOutcome } from 'poki/src/types/walletConnect'
import { WidgetEvent, WidgetType } from 'poki/src/types/widgets'
...
type PokiXTransactionResultProperties = BaseSwapTransactionResultProperties & {
  order_hash: string
}

type BridgeSwapTransactionResultProperties = BaseSwapTransactionResultProperties

type FailedPokiXOrderResultProperties = Omit<PokiXTransactionResultProperties, 'hash'>
...
export type UniverseEventProperties = {
  ...
  [InterfaceEventName.POKI_WALLET_MICROSITE_OPENED]: ITraceContext
  [InterfaceEventName.POKI_WALLET_APP_DOWNLOAD_OPENED]: ITraceContext & {
  ...
  [InterfaceEventNameLocal.PokiXOrderDetailsSheetOpened]: {
    order: string
  }
  [InterfaceEventNameLocal.PokiXOrderCancelInitiated]: {
    orders: string[]
  }
  ...
  [InterfaceEventNameLocal.NoQuoteReceivedFromRoutingAPI]: {
    requestBody: unknown
    response: unknown
    routerPreference: 'price' | 'pokix' | 'api'
  }
  [InterfaceEventNameLocal.PokiXSignatureDeadlineExpired]: {
    deadline: number
    resultTime: number
  }
  [InterfaceEventNameLocal.PokiXSignatureRequested]: Record<string, unknown> // TODO specific type
  [InterfaceEventNameLocal.PokiXOrderPostError]: Record<string, unknown> // TODO specific type
  [InterfaceEventNameLocal.PokiXOrderSubmitted]: Record<string, unknown> // TODO specific type
  ...
  [PokiEventName.BalancesReport]: {
    total_balances_usd: number
    wallets: string[]
    balances: number[]
  }
  [PokiEventName.BalancesReportPerChain]: {
    total_balances_usd_per_chain: Record<string, number>
    wallet: string
  }
  [PokiEventName.ConversionEventSubmitted]: {
    id: string
    eventId: string
    eventName: string
  ...
  [PokiEventName.BlockaidFeesMismatch]: {
    symbol: string
    address: string
    chainId: number
