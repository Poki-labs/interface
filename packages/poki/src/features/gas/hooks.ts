...
import { useAccountMeta, useProvider } from 'poki/src/contexts/PokiContext'
import { useGasFeeQuery } from 'poki/src/data/apiClients/pokiApi/useGasFeeQuery'
import { GasEstimate, GasStrategy } from 'poki/src/data/tradingApi/types'
...
import {
  FormattedPokiXGasFeeInfo,
  GasFeeResult,
  TransactionEip1559FeeParams,
  TransactionLegacyFeeParams,
  areEqualGasStrategies,
} from 'poki/src/features/gas/types'
import { hasSufficientFundsIncludingGas } from 'poki/src/features/gas/utils'
import { DynamicConfigs, GasStrategies, GasStrategyType } from 'poki/src/features/gating/configs'
import { Statsig, useConfig } from 'poki/src/features/gating/sdk/statsig'
...
import { usePollingIntervalByChain } from 'poki/src/features/transactions/swap/hooks/usePollingIntervalByChain'
import { useUSDCValueWithStatus } from 'poki/src/features/transactions/swap/hooks/useUSDCPrice'
 ...
 *
 * More info: https://www.notion.so/pokilabs/Gas-Limit-Experiment-14ac52b2548b80ea932ff2edfdab6683
 *
...
export function useFormattedPokiXGasFeeInfo(
  pokiXGasBreakdown: PokiXGasBreakdown | undefined,
  chainId: UniverseChainId,
): FormattedPokiXGasFeeInfo | undefined {
  const { convertFiatAmountFormatted } = useLocalizationContext()

  const { value: approvalCostUsd } = useUSDValueOfGasFee(chainId, pokiXGasBreakdown?.approvalCost)
  const { value: wrapCostUsd } = useUSDValueOfGasFee(chainId, pokiXGasBreakdown?.wrapCost)

  return useMemo(() => {
    if (!pokiXGasBreakdown) {
      return undefined
    }
    const { approvalCost, wrapCost, inputTokenSymbol } = pokiXGasBreakdown
    // Without pokix, the swap would have costed approval price + classic swap fee. A separate wrap tx would not have occurred.
    const preSavingsGasCostUsd = Number(approvalCostUsd ?? 0) + Number(pokiXGasBreakdown?.classicGasUseEstimateUSD ?? 0)
  ...
  }, [pokiXGasBreakdown, approvalCostUsd, convertFiatAmountFormatted, wrapCostUsd])
}
