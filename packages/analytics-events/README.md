# Poki Labs Analytics Events
[![npm](https://img.shields.io/npm/v/analytics-events/src/index)](https://www.npmjs.com/package/analytics-events/src/index)

The `analytics-events/src/index` package is a [npm package](https://www.npmjs.com/package/analytics-events/src/index) of constants used as the inputs into our [@poki/analytics](https://www.npmjs.com/package/@poki/analytics) package ([Github Link](https://github.com/Poki/analytics)). It is used in repos such as [interface](https://github.com/Poki/interface) and [docs](https://github.com/Poki/docs).
...
```bash
yarn add analytics-events/src/index
```

```bash
npm i --save analytics-events/src/index
```
...
The [analytics](https://github.com/Poki/analytics) library implements the [Trace framework](https://slack.engineering/creating-a-react-analytics-logging-library/) to enable easy default properties and basic hierarchy of context. The following context properties are available, in order of specificity. This can be used directly for ease or logged manually for custom trigger events.
...
`git restore package.json yarn.lock && rm poki-analytics-events-dev.tgz && yarn`.
...
This will generate a `poki-analytics-events-dev.tgz` including your changes and a `0.0.1` package version number.
...
# yarn
yarn cache clean
yarn add file:poki-analytics-dev.tgz

# npm
npm install poki-analytics-dev.tgz
```
...
Poki Labs encourages integrators to evaluate their own regulatory obligations when integrating this package into their products, including, but not limited to, those related to economic or trade sanctions compliance.
