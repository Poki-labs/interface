...
The `ui` package contains all low level components that are shared between apps. It should *not* contain components that are specific to any one app or Poki business logic. Each component should be guided by the following principles:
...
Components that are shared between all applications but encode Poki business logic should most likely be placed in the `poki` package!
