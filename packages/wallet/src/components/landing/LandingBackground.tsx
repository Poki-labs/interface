...
import { EXTENSION_LOADING_BACKGROUND, POKI_LOGO } from 'ui/src/assets'
...
const OnboardingAnimation = ({
  ...
  const pokiLogoScale = useSharedValue(1.5)
  const animatedStyle = useAnimatedStyle(() => ({ transform: [{ scale: pokiLogoScale.value }] }), [pokiLogoScale])

  useEffect(() => {
    pokiLogoScale.value = withDelay(
      LOGO_SCALE_DELAY,
      withTiming(1, {
...
export function ExtensionLoadingBackground() {
  return <Image height={400} resizeMode="contain" source={EXTENSION_LOADING_BACKGROUND} width={458} />
}
