...
import { OnRampTransactionsAuth } from 'poki/src/data/graphql/poki-data-api/__generated__/types-and-hooks'
import { objectToQueryString } from 'poki/src/data/utils'
...
export async function createSignedRequestBody<T>(
  ...
  const requestBody: T & AuthData = {
    ...data,
    'x-poki-address': account.address,
    'x-poki-timestamp': Date.now(),
  }
