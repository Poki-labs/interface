...
import { getValidAddress, sanitizeAddressText } from 'poki/src/utils/addresses'
...
import { AccountCredentials } from 'wallet/src/features/wallet/Keyring/types'
 ...
 * @param options.includeUnitagSuffix - Whether to include the unitag suffix (.poki.eth) in returned unitag name
...
export function usePrincipalDisplayName(address: Maybe<string>, options?: DisplayNameOptions): DisplayName | undefined {
  const defaultOptions = {
    showShortenedEns: false,
    includeUnitagSuffix: false,
    showLocalName: true,
...
export function useActiveAccountIdentity() {
  const activeSignerAccount = useActiveSignerAccount()
  const [accountCredentials, setAccountCredentials] = useState<AccountCredentials | undefined>(undefined)

  useEffect(() => {
    async function call() {
      if (activeSignerAccount) {
        const accountCredentials = await Keyring.getAccountCredentialsFromMnemonicId(
          activeSignerAccount.mnemonicId,
          activeSignerAccount.derivationIndex,
        )
