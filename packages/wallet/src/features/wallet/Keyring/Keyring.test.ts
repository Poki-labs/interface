...
const ENCRYPTION_KEY_KEY = 'com.poki.web.encryptionKey'
...
describe(WebKeyring, () => {
  ...
  describe('getMnemonicIds', () => {
    it('returns all mnemonic ids', async () => {
      await mockStore({
        data: {
          'com.poki.web.mnemonic.address1': SAMPLE_SEED,
          'com.poki.web.mnemonic.address2': SAMPLE_SEED,
          'com.poki.web.privateKey.address3': 'private-key',
        },
      })
  ...
  describe('unlock', () => {
    ...
    it('fails when password is valid, but stored mnemonic is not', async () => {
      await mockStore({
        data: {
          [`com.poki.web.mnemonic.${SAMPLE_SEED_ADDRESS_1}`]: JSON.stringify({
            ciphertext: 'fail',
          }),
        },
      })
