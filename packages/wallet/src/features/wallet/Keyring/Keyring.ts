...
export interface IKeyring {
  ...
  getAccountCredentialsFromMnemonicId(mnemonicId: string, subaccount?: number): Promise<AccountCredentials>
}
...
class NullKeyring implements IKeyring {
  ...
  getAccountCredentialsFromMnemonicId(_mnemonicId: string, _subaccount?: number): Promise<AccountCredentials> {
    throw new PlatformSplitStubError('getAccountCredentialsFromMnemonicId')
  }
}
