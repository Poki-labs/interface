...
import { CopyAlt, ExternalLink, PokiX } from 'ui/src/components/icons'
    ...
    case TransactionType.Swap:
      if (isShowingMore) {
        ...
        if (
          ...
          specificRows.push(<PokiFeeRow key="pokiFee" typeInfo={typeInfo} />)
        }
      }
      break
...
function NetworkFeeRow({ transactionDetails }: { transactionDetails: TransactionDetails }): JSX.Element {
  ...
  const Logo = isPokiX(transactionDetails) ? PokiX : NetworkLogo
  const GasText = isPokiX(transactionDetails) ? PokiXText : Text
  return (
...
function PokiFeeRow({ typeInfo }: { typeInfo: SwapTypeTransactionInfo }): JSX.Element {
  const { t } = useTranslation()
  const formatter = useLocalizationContext()
  ...
  // Using the equation (1 - 0.25 / 100) * (actualOutputValue + pokiFee) = actualOutputValue
  const approximateFee = amountExact ? (POKI_FEE / (1 - POKI_FEE)) * amountExact : null
  ...
  return (
    <InfoRow label={t('transaction.details.pokiFee', { feePercent: POKI_FEE * 100 })}>
