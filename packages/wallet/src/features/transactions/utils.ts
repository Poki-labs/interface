...
import { isBridge, isPokiX } from 'poki/src/features/transactions/swap/utils/routing'
import {
...
export function getIsCancelable(tx: TransactionDetails): boolean {
  ...
  if (tx.status === TransactionStatus.Pending && (isPokiX(tx) || Object.keys(tx.options?.request).length > 0)) {
    return true
  }
  return false
}
