...
import { makeSelectTransaction, useSelectAddressTransactions } from 'poki/src/features/transactions/selectors'
import { finalizeTransaction } from 'poki/src/features/transactions/slice'
...
import { ensureLeading0x } from 'poki/src/utils/addresses'
import { areCurrencyIdsEqual, buildCurrencyId } from 'poki/src/utils/currencyId'
...
export type ErroredQueuedOrder = PokiXOrderDetails & {
...
export function useMergeLocalAndRemoteTransactions(
  ...
  return useMemo((): TransactionDetails[] | undefined => {
    ...
    // This map enables `getTrackingHash` to deduplicate PokiX orders in the event that one source
    ...
    function populateOrderHashToTxHashMap(tx: TransactionDetails): void {
      if (isPokiX(tx) && tx.hash && tx.orderHash) {
    ...
    function getTrackingHash(tx: TransactionDetails): string | undefined {
      ...
      } else if (isPokiX(tx) && tx.orderHash) {
        const orderHash = ensureLeading0x(tx.orderHash.toLowerCase())
