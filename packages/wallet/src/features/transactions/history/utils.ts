...
import { FORMAT_DATE_MONTH, FORMAT_DATE_MONTH_YEAR, LocalizedDayjs } from 'poki/src/features/language/localizedDayjs'
...
import { CurrencyIdToVisibility, NFTKeyToVisibility } from 'poki/src/features/visibility/slice'
import { Token } from 'poki/src/sdk-core'
...
import { extractPokiXOrderDetails } from 'wallet/src/features/transactions/history/conversion/extractOrderDetails'
...
export function formatTransactionsByDate(
  ...
  const pendingSorted = pending.sort((a, b) => {
    // sort based on timestamp if a PokixX order is present, since pending PokiX orders do not have a nonce.
    if (isPokiX(a) || isPokiX(b)) {
...
export function parseDataResponseToTransactionDetails(
  ...
  if (data.portfolios?.[0]?.assetActivities) {
    return data.portfolios[0].assetActivities.reduce((accum: TransactionDetails[], t) => {
      ...
      } else if (t?.details?.__typename === TransactionDetailsType.PokiXOrder) {
        const parsed = extractPokiXOrderDetails(t as TransactionListQueryResponse)
        if (parsed) {
          accum.push(parsed)
        }
      }
