...
import { fetchSwaps } from 'poki/src/data/apiClients/tradingApi/TradingApiClient'
import { SwapStatus } from 'poki/src/data/tradingApi/__generated__'
import { UniverseChainId } from 'poki/src/features/chains/types'
import { FORTransactionDetails } from 'poki/src/features/fiatOnRamp/types'
import { getGasPrice } from 'poki/src/features/gas/types'
import { findLocalGasStrategy } from 'poki/src/features/gas/utils'
import { DynamicConfigs, MainnetPrivateRpcConfigKey } from 'poki/src/features/gating/configs'
import { getDynamicConfigValue } from 'poki/src/features/gating/hooks'
import { pushNotification, setNotificationStatus } from 'poki/src/features/notifications/slice'
import { AppNotificationType } from 'poki/src/features/notifications/types'
import { refetchGQLQueries } from 'poki/src/features/portfolio/portfolioUpdates/refetchGQLQueriesSaga'
import {
  FLASHBOTS_DEFAULT_BLOCK_RANGE,
  waitForFlashbotsProtectReceipt,
} from 'poki/src/features/providers/FlashbotsRpcProvider'
import { MobileAppsFlyerEvents, WalletEventName } from 'poki/src/features/telemetry/constants'
...
} from 'poki/src/features/transactions/selectors'
import {
  addTransaction,
  cancelTransaction,
...
} from 'poki/src/features/transactions/slice'
import { tradeRoutingToFillType } from 'poki/src/features/transactions/swap/analytics'
...
import { toTradingApiSupportedChainId } from 'poki/src/features/transactions/swap/utils/tradingApi'
import {
...
import { currencyIdToChain } from 'poki/src/utils/currencyId'
...
export function* transactionWatcher({ apolloClient }: { apolloClient: ApolloClient<NormalizedCacheObject> }) {
  ...
  for (const transaction of incompleteTransactions) {
    if (isFORTransaction(transaction)) {
      ...
      // If the transaction was a queued PokiX order that never became submitted, update UI to show failure
      if (isPokiX(transaction) && transaction.queueStatus === QueuedOrderStatus.Waiting) {
...
export function* watchTransaction({
  ...
  const options = isPokiX(transaction) ? undefined : transaction.options
  ...
  // `cancelTx` and `updatedTransaction` conditions apply to both Classic and PokiX transactions
  if (cancelTx) {
  ...
  // `replace` and `invalidated` conditions do not apply to PokiX orders
...
function* waitForRemoteUpdate(transaction: TransactionDetails, provider: providers.Provider) {
  ...
  // For PokiX orders, we need to wait for the order to be filled before we can get the hash
  if (isPokiX(transaction) && transaction.orderHash && transaction.queueStatus) {
  ...
  // Classic transaction status is based on receipt, while PokiX status is based backend response.
...
export function* waitForSameNonceFinalized(chainId: UniverseChainId, id: string, nonce: BigNumberish | undefined) {
  while (true) {
    ...
    if (
      !isPokiX(payload) && // PokiX transactions are submitted by a filler, so they cannot invalidate a transaction sent by a user.
...
export function logTransactionEvent(actionData: ReturnType<typeof transactionActions.finalizeTransaction>): void {
  ...
  if (type === TransactionType.Swap || type === TransactionType.Bridge) {
    ...
    const baseProperties = {
      ...
      gasUseEstimate,
      quoteId,
      submitViaPrivateRpc: isPokiX(payload) ? false : payload.options.submitViaPrivateRpc,
      transactedUSDValue,
      ...swapProperties,
      ...bridgeProperties,
    }
    ...
    if (isPokiX(payload)) {
      ...
      // All local pokix swaps should be tracked in redux with an orderHash .
      if (!orderHash) {
        logger.error(new Error('Attempting to log pokix swap event without a orderHash'), {
          tags: {
            file: 'transactionWatcherSaga',
