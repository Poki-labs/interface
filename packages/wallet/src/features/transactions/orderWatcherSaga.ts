import { fetchOrders } from 'poki/src/data/apiClients/tradingApi/TradingApiClient'
import { GetOrdersResponse, OrderStatus } from 'poki/src/data/tradingApi/__generated__/index'
import { makeSelectPokiXOrder } from 'poki/src/features/transactions/selectors'
...
export class OrderWatcher {
  private static listeners: {
    [orderHash: string]: {
      updateOrderStatus: (updatedOrder: PokiXOrderDetails) => void
      promise: Promise<PokiXOrderDetails>
    }
  } = {}
  ...
  private static *poll(index: number): Generator<unknown> {
    ...
    try {
      ...
      for (const localOrderHash of orderHashes) {
        ...
        const selectPokiXOrder = yield* call(makeSelectPokiXOrder)
        const localOrder = yield* select(selectPokiXOrder, { orderHash: localOrderHash })
  ...
  static *waitForOrderStatus(orderHash: string, queueStatus: QueuedOrderStatus) {
    ...
    let resolvePromise: (value: PokiXOrderDetails) => void
    const promise = new Promise<PokiXOrderDetails>((resolve) => {
