...
import { usePortfolioCacheUpdater } from 'poki/src/features/dataApi/balances'
...
import { useDeleteTagTokens } from 'poki/src/features/tokens/slice/hooks'
import { CurrencyField, CurrencyId } from 'poki/src/types/currency'
import { areCurrencyIdsEqual, currencyIdToAddress, currencyIdToChain } from 'poki/src/utils/currencyId'
import { Eye, EyeOff, ReceiveAlt, SendAction } from 'ui/src/components/icons'
...
export function useTokenContextMenu({
  ...
  const activeAccountHoldsToken =
    portfolioBalance && areCurrencyIdsEqual(currencyId, portfolioBalance?.currencyInfo.ledgerId)
  ...
  const onPressSend = useCallback(() => {
    ...
    navigateToSend({ currencyAddress: currencyId, chainId: currencyChainId })
  }, [currencyId, currencyChainId, navigateToSend])
  ...
  }, [deleteTagTokens, updateCache, isVisible, portfolioBalance, currencyId, dispatch, tokenSymbolForNotification, t])
  ...
  const menuActions = useMemo(() => {
    const allMenuActions: MenuAction[] = [
      ...
      //     ]
      //   : []),
      ...(activeAccountHoldsToken && !isTestnetModeEnabled && !DEFAULT_TAG_TOKENS.includes(currencyId)
        ? [
            {
